...
# ---------------------------------------------------------------------------
# Public entry point — legacy single-shot
# ---------------------------------------------------------------------------
def convert_file_to_vector_outputs(filename: str, settings: VectorSettings) -> dict:
    """
    Convert an uploaded image into both an SVG and a PDF in one shot.
    Used by the legacy single-page UI.
    """
    ...
    try:
        ...
        # Phase 1D — palette becomes available after preprocessing
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to convert image: {ex}",
        )
